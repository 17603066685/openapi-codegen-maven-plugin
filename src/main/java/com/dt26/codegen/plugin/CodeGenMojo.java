...
public class CodeGenMojo extends AbstractMojo {
    ...
    public void execute() throws MojoExecutionException {
        ...
        if (inputSpecFile.isDirectory()) {
            File[] files = inputSpecFile.listFiles();
            for (int i = 0; i < files.length; i++) {
                if (files[i].isFile()) {
                    String path = files[i].getPath();
                    getLog().info(i + ": " + path);
                    this.executeHandle(files[i], path);
                }
            }
        }
    }

    private void executeHandle(File yamlFile, String path) throws MojoExecutionException {
        ...
        try {
            ...
            if (buildContext != null) {
                if (buildContext.isIncremental()) {
                    if (inputSpec != null) {
                        if (yamlFile.exists()) {
                            if (!buildContext.hasDelta(yamlFile)) {
                                getLog().info(
            ...
            if (skipIfSpecIsUnchanged) {
                if (yamlFile.exists()) {
                    File storedInputSpecHashFile = getHashFile(yamlFile);
                    if (storedInputSpecHashFile.exists()) {
                        String inputSpecHash = Files.asByteSource(yamlFile).hash(Hashing.sha256()).toString();
            ...
            if (isNotEmpty(path)) {
                configurator.setInputSpec(path);
            }
            ...
            ByteSource inputSpecByteSource =
                    yamlFile.exists()
                            ? Files.asByteSource(yamlFile)
                            : CharSource.wrap(ClasspathHelper.loadFileFromClasspath(yamlFile.toString().replaceAll("\\\\", "/")))
            ...
            if (buildContext != null) {
                buildContext.addError(yamlFile, 0, 0, "unexpected error in Open-API generation", e);
            }
            getLog().error(e);
            throw new MojoExecutionException(
